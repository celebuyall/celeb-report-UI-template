...
- theme 순환 배정: 1번 → 'green', 2번 → 'blue', 3번 → 'pink', 4번 → 'green', ...
...
const contentIdeas: ContentIdea[] = [
  {
    ...
    theme: 'green' | 'blue' | 'pink';  // 순환 배정: 1→green, 2→blue, 3→pink, 4→green, ...
  }
];
...
export const productDefinition = {
  ...
  keyConcepts: [
    // 3개 - 서론에서 핵심 가치 제안 추출
    // { icon: <아이콘 size={24} />, title: "...", description: "...", theme: "green" },
...
export const contentIdeas: ContentIdea[] = [
  ...
  // theme 순환: green → blue → pink → green → ...
...
- [ ] contentIdeas의 theme이 순환 배정되었는가? (green→blue→pink→green→...)
- [ ] strategicPillars가 3개이고, theme이 green/blue/pink 순서인가?
- [ ] 모든 아이콘이 목록에 있는 것인가?
